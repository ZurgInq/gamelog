...
func main() {
	dirInfo, err := os.ReadDir(pagesDir)
	if err != nil {
		log.Fatal(err)
	}
...
func compilePage(tmpl *template.Template, pageName string) (string, error) {
	...
	if output, err = os.Create(dest); err != nil {
		return dest, fmt.Errorf("create file %s: %w", pageName, err)
	}
	if err := tmpl.ExecuteTemplate(output, "base", nil); err != nil {
		return dest, fmt.Errorf("execute template: %w", err)
	}
